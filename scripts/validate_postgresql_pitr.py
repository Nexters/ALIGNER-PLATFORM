...
SECRET_MARKERS = re.compile(r"(authorization|credential|password|secret|token|access[_-]?(?:key|token)|private[_-]?key)", re.I)
...
def validate(result: Any) -> list[str]:
    ...
    required = {
        "issue", "status", "started_at_utc", "target_time_utc", "production_cluster", "production_namespace",
        "production_pvcs", "restore_cluster", "restore_namespace", "restore_pvcs", "wal_continuity_verified",
        "restore_credential_mode", "baseline", "restored", "schema_match", "marker_match", "row_counts_match",
        "checksum_match", "rto_seconds", "rpo_seconds", "cleanup",
    }
    ...
    for name, value in (("baseline", baseline), ("restored", restored)):
        if (
            ...
            or not isinstance(value.get("checksums"), dict)
        ):
