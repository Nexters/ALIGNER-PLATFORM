...
정책 deny/allow, RSS/CPU/metric series를 기록해야 한다. agent 재시작과 VM 한 대 정지는
외부 파괴 작업이므로 자동화하지 않는다. [Cilium Gate runbook](../runbooks/cilium-gate.md)의
수동 증적이 보호된 외부 검토에서 승인되기 전에는 Gate가 FAIL이다. 어느 하나라도 실패하면
프로덕션 데이터 투입을 막고, 운영 중 CNI 교체 대신 Flannel로 **클러스터를 재생성**할지 ADR을
갱신해 결정한다.
...
| Agent restart | `NOT_EXECUTED`; protected external review evidence | data blocked; capacity/remediation review |
| VM stop | `NOT_EXECUTED`; protected external review evidence | data blocked; Flannel rebuild decision required |
